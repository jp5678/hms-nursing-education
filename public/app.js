...
// Helper delay to simulate server communication latencies
const delay = (ms) => new Promise(resolve => setTimeout(resolve, ms));

// ====================================================
// INITIAL MASTER DATA (Fallback Database)
// ====================================================
const INITIAL_DB = {
  users: [
  ...
  appointments: [
    {
      ...
      patientName: "홍길동",
      doctorId: "doctor1",
      doctorName: "이의사 (순환기내과)",
      date: "2026-06-06",
      time: "10:00",
      symptoms: "최근 3일간 지속적인 두통 및 혈압 상승(150/95mmHg)",
      status: "진료중"
    },
    {
      id: "A-00002",
      ...
      patientName: "이철수",
      doctorId: "doctor2",
      doctorName: "박의사 (소아청소년과)",
      date: "2026-06-06",
      time: "11:30",
      symptoms: "기침, 발열 및 호흡 곤란 증세",
      status: "대기"
    },
    {
      id: "A-00003",
  ...
  consultations: [
    {
      id: "C-00001",
      appointmentId: "A-00003",
      patientId: "P-00002",
      doctorId: "doctor1",
      date: "2026-06-05",
      chiefComplaint: "당뇨 관리 상태 체크",
      diagnosis: "2형 당뇨병 (ICD-10: E11)",
      prescriptions: [
        {
          drugName: "Metformin",
          dosage: "500mg",
  ...
  nursingLogs: [
    {
      ...
      nursingDiagnosis: "높은 혈압 및 알레르기 반응 위험성",
      nursingPlan: "1. 2시간 간격으로 혈압 및 바이탈 사인 측정.\n2. 페니실린계 약물 투여 금기 사항을 침상 머리맡에 부착 및 차트 강조.\n3. 환자에게 알레르기 유발 물질 차단 교육 시행.",
      nursingImplementation: "1. 환자 입원 시 바이탈 측정: BP 155/98, PR 82, RR 18, BT 36.6도.\n2. 페니실린 알레르기 붉은색 팔찌 착용 완료 및 전산 등록 완료.\n3. 식단 확인 및 저염식 교육 완료.",
      nursingEvaluation: "환자는 본인의 페니실린 알레르기 위험성을 인지하고 있으며, 혈압은 1시간 후 재측정 시 145/90mmHg로 소폭 하향 안정화됨."
    }
  ]
};
...
function getLocalDB() {
  let db = localStorage.getItem('hms_db');
  if (!db) {
...
function saveLocalDB(db) {
  localStorage.setItem('hms_db', JSON.stringify(db));
}
...
// ====================================================
// 1. DASHBOARD VIEW (LocalStorage Edition)
// ====================================================
async function loadDashboardView(container) {
  ...
  // Compute specialty distribution dynamically
  ...
  const chartData = {
    registrationTrends: {
      ...
      data: [1, 2, 0, 3, 2, db.patients.length]
    },
...
// ====================================================
// 2. PATIENTS VIEW (LocalStorage Edition)
// ====================================================
...
async function handleAddPatient(e) {
  ...
  const allergies = document.getElementById('p-allergy').value || '없음';
  const notes = document.getElementById('p-notes').value;

  await delay(100);
  const db = getLocalDB();
  const newId = `P-${String(db.patients.length + 1).padStart(5, '0')}`;
  const newPatient = { id: newId, name, birthDate, gender, phone, bloodType, allergies, notes };
...
async function viewPatientDetail(id) {
  ...
  const patientLogs = db.nursingLogs.filter(log => log.patientId === id);
...
// ====================================================
// 3. APPOINTMENTS VIEW (LocalStorage Edition)
// ====================================================
...
async function handleAddAppointment(e) {
  ...
  if (!patient || !doctor) {
    showToast('환자 또는 의사 정보가 유효하지 않습니다.');
    return;
  }
...
// ====================================================
// 4. CONSULTATIONS & PRESCRIPTIONS (LocalStorage Edition)
// ====================================================
async function loadConsultationsView(container) {
  ...
  const activeAppts = db.appointments.filter(a => (a.status === '대기' || a.status === '진료중'));
  const consults = db.consultations;
  const patients = db.patients;
          ...
          <div id="active-consultation-form-container" style="display: none;" class="border-top border-secondary pt-3">
            ...
            <form id="consultation-form">
              ...
              <div class="p-3 bg-dark border border-secondary rounded mb-3">
                ...
                <div id="prescription-rows-container"></div>
              </div>
...
// ====================================================
// 5. BILLINGS VIEW (LocalStorage Edition)
// ====================================================
...
function viewReceipt(id) {
  const db = getLocalDB();
  const b = db.billings.find(bill => bill.id === id);
  if (!b) return;
          ...
          <div class="modal-body p-4 print-page">
            ...
            <table class="table text-light border-secondary small">
              <tbody>
                <tr>
                  <td>환자 성명</td>
                  <td class="text-end fw-bold">${b.patientName} (${b.patientId || '알수없음'})</td>
                </tr>
                <tr>
                  <td>진찰 진료비</td>
...
// ====================================================
// 6. REPORTS VIEW (LocalStorage Edition)
// ====================================================
...
// ====================================================
// 7. STAFF VIEW (LocalStorage Edition)
// ====================================================
